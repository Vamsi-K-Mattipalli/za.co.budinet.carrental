package TestCases.SalesMarketing.Renter;

import config.DataProviders;
import config.TestCore;
import controlers.Generics;
import io.qameta.allure.*;
import org.testng.annotations.AfterMethod;
import org.testng.annotations.BeforeMethod;
import org.testng.annotations.Test;
import pageLibrary.CloseWebDriver;
import pageLibrary.LogIn;
import pageLibrary.Menu;

/**
 * Created by GregDev on 2017/05/16.
 */
public class TCSR15 extends TestCore {

    @BeforeMethod
    public void setUp() throws Exception {
        init();
    }
    @Epic("SALES")
    @Feature("MARKERTING")
    @Story("RENTER")
    @Severity(SeverityLevel.NORMAL)
    @Test(dataProvider = "login", dataProviderClass = DataProviders.class)
    public void TCSR15_Test1(String Username, String Password) throws Exception {
        LogIn.SignIn(Username, Password);
        Menu.NavigateTo3("salesmarketing.menu.sales", "salesmarketing.menu.renter", "salesmarketing.menu.renter.enquiry");
        driverwait();
        Generics.DropdownSelector("Renter Type", "renter.dropdown.RENTERIDTYPE", "SA ID");
        Generics.ClickButtonLink("loyalty.button.Submit", "loyalty.button.Submit");
        Generics.ClickButtonLink("loyalty.link.renterid", "loyalty.link.renterid");
        driver.switchTo().frame("MainTabs");
        driver.switchTo().frame(1);
        Generics.ClickButtonLink("loyalty.button.modify", "loyalty.button.modify");
        Generics.FieldPopulate("Effective To Date", "renter.field.efftodate", "2018-01-01");
        Generics.ClickButtonLink("loyalty.button.Submit", "loyalty.button.Submit");
    }

    @AfterMethod
    public void tearDown() throws Exception {
        CloseWebDriver.browser();

    }
}
